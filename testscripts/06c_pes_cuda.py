...
__global__ void apply_gradient(
  ...
  for (int ilayer = 1; ilayer < nlayers; ilayer++) {
    ...
    if ( inode < size ) {
      ...
      biases[inode + biases_offset] -= ( training_rate / ( (float) batch_size) ) * biases_grad[inode + biases_offset];
      biases_grad[inode + biases_offset] = 0.0f;
      for ( int iprev = 0; iprev < size_prev; iprev++ ) {
        weights[ ( inode * size_prev ) + iprev + weights_offset ] -= ( training_rate / ( (float) batch_size) ) * weights_grad[ ( inode * size_prev ) + iprev + weights_offset ];
        weights_grad[ ( inode * size_prev ) + iprev + weights_offset ] = 0.0f;
      }
    }
...
apply_gradient_gpu = cuda_module.get_function('apply_gradient')
        ...
        for iepoch in range(nepochs):
            ...
            for istart in range(0, ninputs, batch_size):
                ...
                for ilayer in range(1, len(self.sizes)):
                    layer = self.layers[ilayer]
                    for inode in range( layer.size ):
                        layer.biases[inode] = self.biases[inode + biases_offset]
                        layer.biases_grad[inode] = self.biases_grad[inode + biases_offset]
                        for jnode in range( self.layers[ilayer-1].size ):
                            layer.weights[inode][jnode] = self.weights[(inode * self.layers[ilayer-1].size) + jnode + weights_offset]
                            layer.weights_grad[inode][jnode] = self.weights_grad[(inode * self.layers[ilayer-1].size) + jnode + weights_offset]
                    biases_offset += layer.size
                    weights_offset += layer.size * self.layers[ilayer-1].size
